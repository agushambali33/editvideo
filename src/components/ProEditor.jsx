...
export default function ProEditor({ setOuterStatus }) {
  ...
  const loadFfmpeg = async () => {
    ...
    const ffmpeg = createFFmpeg({ log: true, progress: ({ ratio }) => setProgress(`ffmpeg ${Math.round(ratio*100)}%`) });
    await ffmpeg.load();
...
\
  ...
  try {
    ...
    if (w.ok) {
      ...
      ffmpeg.FS('writeFile', 'watermark.png', new Uint8Array(ab));
      watermarkFound = true;
  ...
  if (watermarkFound && enableWatermark) {
    ...
    args.push('-filter_complex', `[0:v]scale=1080:1920:force_original_aspect_ratio=decrease,pad=1080:1920:(ow-iw)/2:(oh-ih)/2,setsar=1[v];[v][1:v]overlay=W-w-10:H-h-10`);
    args.push('-an', '-c:v', 'libx264', '-preset', 'veryfast', '-crf', '23', tmp);
  } else {
  ...
  if (useVoice) {
    ...
    if (ttsRes.data?.audio) {
      ...
      ffmpeg.FS('writeFile', narration, audioBytes);
      setProgress('merging audio');
      await ffmpeg.run('-i', tmp, '-i', narration, '-c:v', 'copy', '-c:a', 'aac', '-map', '0:v:0', '-map', '1:a:0', '-shortest', out);
    } else {
      log('TTS failed or empty, copying video only');
      await ffmpeg.run('-i', tmp, '-c:v', 'copy', '-c:a', 'aac', '-shortest', out);
    }
  } else {
    await ffmpeg.run('-i', tmp, '-c:v', 'copy', '-c:a', 'aac', '-shortest', out);
  }
...
const shareHelpers = async () => {
  ...
  if (navigator.share) {
    try {
      ...
      log('Share cancelled or failed: ' + e.message);
    }
  } else {
...
return (
  <div>
    ...
    <div style={{marginTop:10}} className="controls">
      <label className="kv"><input type="checkbox" checked={enableWatermark} onChange={(e)=>setEnableWatermark(e.target.checked)} /> <span className="small">Watermark</span></label>
      <label className="kv"><input type="checkbox" checked={useVoice} onChange={(e)=>setUseVoice(e.target.checked)} /> <span className="small">AI Voiceover</span></label>
    </div>
